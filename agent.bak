...
const MODEL = process.env.MODEL || 'gpt-4o-mini'; // use outro se preferir
...
// ===== loop principal (Chat Completions) =====
...
- Pare quando os critérios forem atendidos ou ao atingir o limite de passos.`
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    // executar cada tool_call e devolver como mensagens "tool"
    for (const c of calls) {
      const { name, arguments: argStr, id } = c.function;
      let args = {};
